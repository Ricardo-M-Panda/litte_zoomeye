...
int tcp_pack(unsigned short randomport, char sendpacket[], int flag)
{
    ...
    if (flag == -1)
        ...
        ip->daddr = dest_rst_addr.sin_addr.s_addr;
    struct tcphdr* tcp;
    //struct timeval* tval;
    ...
    tcp->check = cal_chksum((unsigned short*)buffers, sizeof(f_tcp)+ packsize);
